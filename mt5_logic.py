...
# def sync_mt5_trades_to_db(user_id, server, login, password, from_date_str, to_date_str):
...
#     # 2. Convert date strings to datetime objects for MT5
#     date_from = datetime.strptime(from_date_str, "%Y-%m-%d")
#     date_to = datetime.strptime(to_date_str, "%Y-%m-%d")

#     # 3. Fetch History (Deals)
#     # history_deals_get filters the data before it even reaches your app
#     deals = mt5.history_deals_get(date_from, date_to)
    
#     if deals is None:
#         mt5.shutdown()
#         return {"error": "Failed to fetch deals from MT5"}

#     # 4. Save to DynamoDB Table: "UserTrades"
#     dynamodb = boto3.resource('dynamodb')
#     trades_table = dynamodb.Table('UserTrades')
    
#     sync_count = 0
#     with trades_table.batch_writer() as batch:
#         for deal in deals:
#             # We use ticket as the unique ID to prevent duplicates
#             batch.put_item(Item={
#                 "user_id": user_id,
...
def fetch_mt5_analytics(server, login, password, from_date, to_date):
    ...
    try:
        ...
        # Convert string dates to datetime objects for MT5
        start_time = datetime.strptime(from_date, "%Y-%m-%d")
        
        # MT5 uses Eastern Summer Time (UTC-4), so we add a 6-hour buffer
        # to the requested end date to ensure all recent trades are captured
        end_time = datetime.strptime(to_date, "%Y-%m-%d") + timedelta(hours=6)

        # UPDATED: Using start_time instead of 0
        deals = mt5.history_deals_get(start_time, end_time) or []
        deals = sorted(deals, key=lambda d: d.time)

        print(f"\n=== FETCHING HISTORY FROM {from_date} TO {to_date} ===")
        print(f"Total deals fetched: {len(deals)}")
