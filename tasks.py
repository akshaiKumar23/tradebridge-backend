...
def get_account_summary(self, user_id, server, login, password, from_date, to_date):
    ...
    # ADDED from_date AND to_date TO THE CALL BELOW
    result = fetch_mt5_analytics(server, login, password, from_date, to_date)
    ...
    return {
        ...
        "summary": {
            ...
            "profit_factor": normalized["profit_factor"],
            "expectancy": normalized["expectancy"],
            "avg_win": normalized["avg_win"],
            "avg_loss": normalized["avg_loss"],
        }
    }